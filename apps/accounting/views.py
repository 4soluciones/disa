...
def cashflow_list(request):
    ...
    if request.method == 'GET':
        ...
        # Obtener la caja principal de la sucursal del usuario por defecto
        user_cash_account = None
        is_cash_opened = False
        
        if hasattr(request.user, 'subsidiary') and request.user.subsidiary:
            # Buscar la caja principal (Soles) de la sucursal del usuario
            user_cash_accounts = Cash.objects.filter(
                subsidiary=request.user.subsidiary,
                currency_type='S'
            ).order_by('id')
            
            if user_cash_accounts.exists():
                user_cash_account = user_cash_accounts.first()
                
                # Verificar si la caja está abierta hoy
                today = datetime.now().date()
                opening_today = CashFlow.objects.filter(
                    cash=user_cash_account,
                    type='A',
                    transaction_date__date=today
                ).exists()
                
                closing_today = CashFlow.objects.filter(
                    cash=user_cash_account,
                    type='C',
                    transaction_date__date=today
                ).exists()
                
                # La caja está abierta si hay apertura y no hay cierre hoy
                is_cash_opened = opening_today and not closing_today
        ...
        return render(request, 'accounting/cashflow_list.html', {
            ...
            'subsidiary_set': subsidiary_set,
            'date_now': date_now,
            'user_cash_account': user_cash_account,
            'is_cash_opened': is_cash_opened,
        })
    elif request.method == 'POST':
        try:
            ...
            # Si no se especifica una cuenta, usar la de la sucursal del usuario por defecto
            if not cash_id or cash_id == '0':
                if hasattr(request.user, 'subsidiary') and request.user.subsidiary:
                    ...
                    if user_cash_accounts.exists():
                        cash_id = str(user_cash_accounts.first().id)
            ...
            # Filtros de fecha - convertir strings a datetime
            if date_from:
                try:
                    date_from_obj = datetime.strptime(date_from, '%Y-%m-%d').date()
                    cashflows = cashflows.filter(transaction_date__date__gte=date_from_obj)
                except ValueError:
                    pass  # Si la fecha no es válida, ignorar el filtro
            if date_to:
                try:
                    date_to_obj = datetime.strptime(date_to, '%Y-%m-%d').date()
                    cashflows = cashflows.filter(transaction_date__date__lte=date_to_obj)
                except ValueError:
                    pass  # Si la fecha no es válida, ignorar el filtro

            # Ordenar: primero aperturas (A), luego por fecha ascendente, y cierres (C) al final
            from django.db.models import Case, When, IntegerField
            cashflows = cashflows.select_related('cash', 'user', 'cash__subsidiary', 'order').annotate(
                type_order=Case(
                    When(type='A', then=0),  # Aperturas primero
                    When(type='C', then=2),  # Cierres al final
                    default=1,  # Resto en el medio
                    output_field=IntegerField()
                )
            ).order_by('type_order', 'transaction_date')
            ...
            # Verificar si la caja está abierta (para el botón)
            is_cash_opened = False
            if cash_id and cash_id != '0':
                try:
                    cash_obj = Cash.objects.get(id=int(cash_id))
...
@csrf_exempt
def cash_opening_save(request):
    """Vista para guardar apertura de caja"""
    if request.method == 'POST':
        try:
            cash_id = request.POST.get('cash_id')
            transaction_date = request.POST.get('transaction_date')
            ...
            # Verificar si ya existe una apertura para esta caja en esta fecha
            opening_date = datetime.strptime(transaction_date, '%Y-%m-%d').date()
            existing_opening = CashFlow.objects.filter(
                cash=cash_obj,
                type='A',
                transaction_date__date=opening_date
            ).first()
            
            if existing_opening:
                return JsonResponse({
                    'success': False,
                    'message': f'Ya existe una apertura de caja para {cash_obj.name} en la fecha {transaction_date}'
                }, status=HTTPStatus.BAD_REQUEST)
            
            # Crear registro de apertura
            opening_datetime = datetime.combine(opening_date, datetime.min.time())
            cashflow_obj = CashFlow(
                transaction_date=opening_datetime,
            ...
            return JsonResponse({
                'success': True,
                'message': f'Apertura de caja registrada exitosamente para {cash_obj.name}',
                'cashflow_id': cashflow_obj.id
            }, status=HTTPStatus.OK)
            ...
            return JsonResponse({
                'success': False,
                'message': 'Cuenta/caja no encontrada'
            }, status=HTTPStatus.NOT_FOUND)
        except Exception as e:
            return JsonResponse({
                'success': False,
                'message': f'Error al registrar la apertura de caja: {str(e)}'
            }, status=HTTPStatus.INTERNAL_SERVER_ERROR)
...
@csrf_exempt
def cash_closing_calculate(request):
    """Vista para calcular el resumen de caja antes del cierre"""
    if request.method == 'POST':
        try:
            cash_id = request.POST.get('cash_id')
            transaction_date = request.POST.get('transaction_date')
            
            if not cash_id or not transaction_date:
                return JsonResponse({
                    'success': False,
                    'message': 'Debe seleccionar una cuenta/caja y una fecha'
                }, status=HTTPStatus.BAD_REQUEST)
            
            # Obtener objetos relacionados
            cash_obj = Cash.objects.get(id=int(cash_id))
            closing_date = datetime.strptime(transaction_date, '%Y-%m-%d').date()
            
            # Obtener apertura de caja para esta fecha
            opening = CashFlow.objects.filter(
                cash=cash_obj,
                type='A',
                transaction_date__date=closing_date
            ).first()
            
            opening_amount = opening.total if opening else Decimal('0.00')
            
            # Obtener todas las entradas (E) y salidas (S) del día
            # Excluir aperturas (A) y cierres (C) previos
            entries = CashFlow.objects.filter(
                cash=cash_obj,
                type='E',
                transaction_date__date=closing_date
            ).exclude(id=opening.id if opening else None)
            
            exits = CashFlow.objects.filter(
                cash=cash_obj,
                type='S',
                transaction_date__date=closing_date
            )
            ...
            # Calcular totales
            total_income = sum(entry.total for entry in entries) if entries.exists() else Decimal('0.00')
            total_expenses = sum(exit.total for exit in exits) if exits.exists() else Decimal('0.00')
            
            # Calcular saldo final: apertura + entradas - salidas
            final_balance = opening_amount + total_income - total_expenses
            ...
            return JsonResponse({
                'success': False,
                'message': f'Error al calcular el resumen de caja: {str(e)}'
            }, status=HTTPStatus.INTERNAL_SERVER_ERROR)
...
@csrf_exempt
def cash_closing_save(request):
    """Vista para guardar cierre de caja"""
    if request.method == 'POST':
        try:
            cash_id = request.POST.get('cash_id')
            transaction_date = request.POST.get('transaction_date')
            amount = request.POST.get('amount', '0.00')
            description = request.POST.get('description', 'CIERRE DE CAJA')
            ...
            # Verificar si ya existe un cierre para esta caja en esta fecha
            closing_date = datetime.strptime(transaction_date, '%Y-%m-%d').date()
            existing_closing = CashFlow.objects.filter(
                cash=cash_obj,
                type='C',
                transaction_date__date=closing_date
            ).first()
            ...
            if existing_closing:
                return JsonResponse({
                    'success': False,
                    'message': f'Ya existe un cierre de caja para {cash_obj.name} en la fecha {transaction_date}'
                }, status=HTTPStatus.BAD_REQUEST)
            
            # Verificar que exista una apertura para esta fecha
            opening = CashFlow.objects.filter(
                cash=cash_obj,
                type='A',
                transaction_date__date=closing_date
            ...
            if not opening:
                return JsonResponse({
                    'success': False,
                    'message': f'No existe una apertura de caja para {cash_obj.name} en la fecha {transaction_date}. Debe abrir la caja primero.'
                }, status=HTTPStatus.BAD_REQUEST)
            ...
            opening_amount = opening.total
            total_income = sum(entry.total for entry in entries) if entries.exists() else Decimal('0.00')
            total_expenses = sum(exit.total for exit in exits) if exits.exists() else Decimal('0.00')
            expected_balance = opening_amount + total_income - total_expenses
            closing_amount = Decimal(str(amount))
            
            # Crear registro de cierre
            closing_datetime = datetime.combine(closing_date, datetime.max.time())
            cashflow_obj = CashFlow(
                transaction_date=closing_datetime,
                created_at=datetime.now(),
                description=description.upper(),
                type='C',  # Cierre
                subtotal=Decimal('0.00'),
                total=closing_amount,
                igv=Decimal('0.00'),
                cash=cash_obj,
                user=user_obj,
                document_type_attached='O'
            ...
            # Mensaje con información del cierre
            message = f'Cierre de caja registrado exitosamente para {cash_obj.name}. '
            message += f'Apertura: S/ {opening_amount:.2f}, Entradas: S/ {total_income:.2f}, Salidas: S/ {total_expenses:.2f}, '
            message += f'Saldo esperado: S/ {expected_balance:.2f}, Saldo cerrado: S/ {closing_amount:.2f}'
            
            if abs(expected_balance - closing_amount) > Decimal('0.01'):
                message += f' (Diferencia: S/ {abs(expected_balance - closing_amount):.2f})'
            ...
            return JsonResponse({
                'success': False,
                'message': f'Error al registrar el cierre de caja: {str(e)}'
            }, status=HTTPStatus.INTERNAL_SERVER_ERROR)
    
    return JsonResponse({'message': 'Error de petición.'}, status=HTTPStatus.BAD_REQUEST)


def check_cash_opening(cash_obj, date):
    """Función helper para verificar si existe una apertura de caja para una fecha específica"""
    if isinstance(date, str):
        check_date = datetime.strptime(date, '%Y-%m-%d').date()
    else:
        check_date = date if hasattr(date, 'date') else date
    
    opening = CashFlow.objects.filter(
        cash=cash_obj,
        type='A',
        transaction_date__date=check_date
    ).first()
